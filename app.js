...
var MinimaxTree = require('./agents/MinimaxTreeSearch').Agent;
...
if (online) {
    ...
    ws.on('message', function receive(data, flags) {
        ...
        if (tag == 'challstr') {
            ...
            if (battles.has(roomid) == false) {
                battles.set(roomid, new InterfaceLayer(roomid, cuser, new WSLayer(this), new MinimaxTree()));
                battleCount--;
                if (battleCount > 0) {
                    for (var format of formats) {
                        ws.send('|/search ' + format);
    ...
    for (var i = 0; i < 1; i++) {
        ...
        scores.push(game.playGames(new RandomAgent(), new MinimaxTree(), 1, 'randommirror'));
